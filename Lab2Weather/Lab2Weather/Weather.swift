...
struct Weather {
    ...
    init(json:[String:Any], city:String, current: [String:Any]) {
        ...
        self.currentTemp = current["temperature"] as! Double
    }

    
    static func getData (latitude: String, longitude: String, city: String, completion: @escaping ([Weather]?) -> ()) {
        
        let API_PATH = "https://api.darksky.net/forecast/23a47f99989a11f4a6a3494dd8c5cbfa/"
        
        let url = API_PATH + "\(latitude),\(longitude)?exclude=minutely,hourly&units=si"
                        ...
                        if let dailyForecasts = json["daily"] as? [String:Any] {
                            if let dailyData = dailyForecasts["data"] as? [[String:Any]] {
                                for dataPoint in dailyData {
                                    if let weatherObject = try? Weather(json: dataPoint, city: city, current: json["currently"] as! [String : Any]) {
